...
export type ReviewSessionEnd = {
  ...
  words: SessionWord[];
  reason: 'size' | 'ranOut';
  /** This session also finished the day — claimed here, shown on the card. */
...
export function startReviewSession(source: ReviewSource, size: number): void {
  current = {
    source,
...
function finish(s: Session, reason: ReviewSessionEnd['reason'], now: number): ReviewSessionEnd {
  current = null;
  const end: ReviewSessionEnd = {
    ...
    words: s.words,
    reason,
    dayDone: claimDayDone(now),
  };
  console.log(
    `[loro:session] ended (${reason}): ${end.correct}/${end.answered} right of ${end.size}` +
      (end.dayDone ? ', day done' : '')
  );
  track('review_ended', {
    source: end.source,
    size: end.size,
    answered: end.answered,
    correct: end.correct,
    reason,
    dayDone: end.dayDone !== null,
  });
  return end;
}
...
export function noteReviewSlide(videoId: string, now: number = Date.now()): ReviewSessionEnd | null {
  ...
  return finish(s, 'ranOut', now);
}

export function raiseReviewEnd(end: ReviewSessionEnd): void {
